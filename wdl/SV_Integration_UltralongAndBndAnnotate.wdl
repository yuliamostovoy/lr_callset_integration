...
task Impl {
    ...
    command <<<
        ...
        function LocalizeSample_Ultralong() {
            ...
            ${TIME_COMMAND} gcloud storage cp ${GCLOUD_STORAGE_BILLING_FLAGS} ${ALIGNED_BAM} ./${SAMPLE_ID}.bam
            gcloud storage cp ${GCLOUD_STORAGE_BILLING_FLAGS} ${ALIGNED_BAI} ./${SAMPLE_ID}.bam.bai
        }
...
END
        ...
        echo "done" > annotate.signal
    >>>

    output {
        String done = read_string("annotate.signal")
    }
    runtime {
        docker: docker_image
        cpu: n_cpu
