...
task Impl {
    ...
    command <<<
        ...
        function Kanpig() {
            ...
            # Counting records marked as present by kanpig without discarding
            # records from the debug output.
            local N_RECORDS_BEFORE_KANPIG=$( bcftools index --nrecords ${SAMPLE_ID}_in.vcf.gz.tbi )
            local N_RECORDS_AFTER_KANPIG=$( bcftools view --include 'GT="alt"' --no-header ${SAMPLE_ID}_in.vcf.gz | wc -l )
...
END
        ...
        while read -u 3 LINE; do
            ...
            bcftools view --output-type b ${SAMPLE_ID}_sv_supp.vcf.gz --output ${SAMPLE_ID}_prekanpig.bcf
            bcftools index --threads ${N_THREADS} -f -c ${SAMPLE_ID}_prekanpig.bcf
            ...
            # Uploading
            gcloud storage mv ${GCLOUD_STORAGE_BILLING_FLAGS} ${SAMPLE_ID}_kanpig.vcf.'gz*' ${SAMPLE_ID}_kanpig.bed.gz ${SAMPLE_ID}_kanpig.csv ${SAMPLE_ID}_training.vcf.'gz*' ${SAMPLE_ID}_prekanpig.'bcf*' ${SAMPLE_ID}_ultralong.'bcf*' ${SAMPLE_ID}_bnd.'bcf*' ~{remote_outdir}/
